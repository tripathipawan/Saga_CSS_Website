...
import { Download, ChevronLeft, ChevronRight, Search, X, CornerDownRight, Eye, Loader2 } from "lucide-react";
...
export const Route = createFileRoute("/cheat-sheet")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Free, printable CSS cheat sheet — flexbox, grid, box model, selectors, units, colors and animation reference in one page." },
      { property: "og:title", content: "CSS Cheat Sheet — SagaCSS" },
      { property: "og:description", content: "The essential CSS reference — flexbox, grid, selectors, units. Download as PDF." },
    ...
    rows: [
      {
        ...
        values: [
          ...
          { name: "<percentage>", meaning: "Relative to the containing block's inline/block size." },
          { name: "auto", meaning: "Browser computes based on content and context." },
          { name: "min-content / max-content / fit-content", meaning: "Intrinsic sizing keywords." },
        ],
        examples: [
          "width: min(90vw, 640px); /* fluid but capped */",
          "height: auto; /* let content decide */",
        ],
        gotcha: "Setting height in % only works if the parent has an explicit height — otherwise it collapses to auto.",
        support: "Universal. Intrinsic keywords (min-content, fit-content) are widely supported in modern browsers.",
      },
      {
        prop: "padding",
        ...
        gotcha: "Percentage padding is resolved against the parent's INLINE size (width) — even for padding-top/bottom.",
      },
      {
        prop: "margin",
      ...
      },
      {
        ...
        values: [
          { name: "border-style", meaning: "none · solid · dashed · dotted · double · groove · ridge · inset · outset" },
        ],
        examples: [
          "border-block-end: 2px dashed currentColor;",
          "border: 0; /* remove all borders */",
        ],
        gotcha: "Forgetting `border-style` (defaults to none) makes width/color silently do nothing.",
      },
      {
        prop: "box-sizing",
        ...
        values: [
          { name: "content-box", meaning: "Default. width/height = content only. Padding & border add on top." },
          { name: "border-box", meaning: "width/height = content + padding + border. Recommended global default." },
        ],
        examples: [
          "*, *::before, *::after { box-sizing: border-box; }",
        ],
        gotcha: "Mixing content-box and border-box across a codebase makes layout math unpredictable — set border-box globally via a universal selector.",
        support: "Universal.",
      },
      {
        prop: "overflow",
        ...
        gotcha: "`overflow: hidden` on an ancestor kills `position: sticky` on descendants — use `overflow: clip` when possible.",
      },
    ],
    realWorld: {
      title: "Product Card",
      description: "A card that uses border-box sizing, padding for internal breathing room, margin for spacing between cards, and a border-radius + subtle border.",
...
.product-card__cta {
  ...
  },
  {
    ...
    rows: [
      { prop: "display", desc: "block · inline · inline-block · flex · grid · none.", example: "display: flex;" },
      ...
      { prop: "position: sticky", desc: "Scrolls until threshold, then sticks.", example: "position: sticky; top: 0;" },
  ...
  },
  {
    ...
    rows: [
      { prop: "display: flex", desc: "Turn the container into a flex layout." },
      {
        ...
        gotcha: "When you switch to column, `justify-content` now controls vertical alignment and `align-items` controls horizontal.",
      },
      {
        prop: "flex-wrap",
      ...
      },
      {
        ...
        values: [
          { name: "stretch / center / start / end / space-between / space-around / space-evenly", meaning: "Same semantics as justify-content but on the cross axis." },
        ],
      },
      {
        prop: "gap",
      ...
      },
      {
        ...
        gotcha: "`flex: 1` uses basis 0 — sizes items purely by ratio. `flex: 1 1 auto` respects content size first, which changes distribution.",
      },
      {
        prop: "align-self",
      ...
      },
      {
        ...
        example: "order: -1;",
        gotcha: "Affects visual order only — screen readers and Tab focus still follow DOM order. Avoid for anything content-critical.",
      },
    ],
    realWorld: {
      title: "Responsive Navbar",
      description: "Logo on the left, nav links centered, actions on the right — using flex + justify-content: space-between + gap.",
      html: `<nav class="nav">
...
.nav__cta {
  ...
  },
  {
    ...
    rows: [
      { prop: "display: grid", desc: "Turn the container into a grid formatting context.", example: "display: grid;" },
      {
        ...
        values: [
          ...
          { name: "auto-fill / auto-fit", meaning: "Create as many tracks as fit. auto-fit collapses empty ones." },
        ],
        examples: [
          "grid-template-columns: repeat(auto-fit, minmax(240px, 1fr));",
          "grid-template-columns: 200px 1fr 200px;",
        ],
        gotcha: "`1fr` distributes FREE space — items with min-content larger than their share can still overflow. Use `minmax(0, 1fr)` to allow shrinking below content width.",
      },
      { prop: "grid-template-rows", desc: "Same syntax as columns but defines row tracks.", example: "grid-template-rows: auto 1fr auto;" },
      {
      ...
      },
      {
        ...
        values: [
          ...
          { name: "dense", meaning: "Backfill gaps left by earlier items — visual order may not match DOM." },
        ],
      },
      { prop: "gap / row-gap / column-gap", desc: "Space between tracks. Works identically to flex gap.", example: "gap: 16px 24px;" },
      {
      ...
      { prop: "place-items", desc: "Shorthand for `align-items justify-items` — controls each item within its cell.", example: "place-items: center;" },
      { prop: "place-content", desc: "Shorthand for `align-content justify-content` — controls the entire grid inside the container.", example: "place-content: center;" },
    ],
    realWorld: {
      title: "Responsive Card Gallery",
      description: "Auto-fitting card grid that reflows from 1 to N columns based on available space, no media queries needed.",
  ...
  },
  {
    ...
    rows: [
      ...
      },
      {
        ...
        gotcha: "Setting size in `px` on the root breaks user zoom for accessibility — use `rem` for body text.",
      },
      {
        prop: "font-weight",
        ...
        gotcha: "Weight only applies if the font actually ships that weight — otherwise the browser fakes it (synthetic bold), which looks worse.",
      },
      { prop: "font-style", desc: "normal · italic · oblique. Italic uses a designed italic face if available; oblique slants the regular face.", example: "font-style: italic;" },
      {
        prop: "line-height",
        ...
        gotcha: "`line-height: 20px` inherits as a fixed 20px even into children with a different font-size. `line-height: 1.5` inherits as the ratio.",
      },
      { prop: "letter-spacing", desc: "Extra space between characters. Tighten headings by a hair (~-0.02em), loosen small uppercase labels.", example: "letter-spacing: 0.05em;" },
      { prop: "text-align", desc: "Horizontal alignment inside a block. `start`/`end` respect writing direction; use them over `left`/`right` for i18n.", example: "text-align: start;" },
      {
      ...
      { prop: "text-transform", desc: "uppercase · lowercase · capitalize. Purely visual — the underlying text stays as authored.", example: "text-transform: uppercase;" },
      {
        prop: "white-space",
      ...
      },
      {
        ...
        gotcha: "For multi-line truncation, use `display: -webkit-box; -webkit-line-clamp: N; -webkit-box-orient: vertical; overflow: hidden;`.",
      },
    ],
    realWorld: {
      title: "Readable Article Body",
      description: "Constrained measure, comfortable line-height, fluid font-size, and truncated multi-line title.",
...
.prose__title {
  ...
  },
  {
    ...
    rows: [
      { prop: "color", desc: "Foreground text color. Inherits by default; `currentColor` is the special keyword that resolves to the element's own color.", example: "color: currentColor;" },
      {
        prop: "rgb() / rgba()",
      ...
      },
      {
        ...
        gotcha: "HSL lightness is NOT perceptually uniform — 50% yellow looks brighter than 50% blue. Use OKLCH for consistent contrast.",
      },
      {
        prop: "oklch() / oklab()",
      ...
      { prop: "background-color", desc: "Solid fill painted BELOW background-image.", example: "background-color: #f8fafc;" },
      {
      ...
      { prop: "background-position", desc: "Anchor of the image. Two keywords or a length pair.", example: "background-position: center bottom;" },
      { prop: "background-repeat", desc: "repeat · no-repeat · repeat-x · repeat-y · round · space." },
      {
      ...
      { prop: "radial-gradient()", desc: "Circular or elliptical gradient. Shape and size are optional first args.", example: "radial-gradient(circle at 20% 20%, #fff, #0000 60%)" },
      { prop: "conic-gradient()", desc: "Sweep around a center point. Perfect for pie charts and color wheels.", example: "conic-gradient(from 0deg, red, yellow, lime, cyan, blue, magenta, red)" },
      {
    ...
    realWorld: {
      ...
      description: "Big hero title using linear-gradient + background-clip: text, over a soft radial-gradient background.",
...
.hero__sub { color: #475569; margin: 0; }`,
  ...
  },
  {
    ...
    rows: [
      { prop: "border-radius", desc: "Round the corners. Per-corner shorthand supported.", example: "border-radius: 12px;" },
      ...
      { prop: "box-shadow", desc: "x y blur spread color · comma-list for multiple layers.", example: "box-shadow: 0 4px 12px rgb(0 0 0 / .2);" },
      { prop: "text-shadow", desc: "x y blur color for text.", example: "text-shadow: 0 1px 2px #000;" },
      { prop: "inset (box-shadow)", desc: "Inner shadow keyword.", example: "box-shadow: inset 0 2px 4px #0003;" },
    ],
  },
  {
    ...
    rows: [
      { prop: "transform", desc: "translate · rotate · scale · skew · matrix.", example: "transform: translateY(-4px) scale(1.05);" },
      { prop: "transform-origin", desc: "Pivot point for transforms." },
      { prop: "perspective", desc: "3D depth applied to children." },
      { prop: "transition", desc: "Shorthand: property duration easing delay.", example: "transition: all .2s ease;" },
      { prop: "transition-timing-function", desc: "ease · linear · ease-in · ease-out · cubic-bezier()." },
      { prop: "animation", desc: "Shorthand: name duration easing delay iter direction fill." },
      { prop: "@keyframes", desc: "Define an animation sequence.", example: "@keyframes fade { from{opacity:0} to{opacity:1} }" },
  ...
  },
  {
    ...
    rows: [
      { prop: ".class", desc: "Elements with the given class. Specificity 0,0,1,0. The workhorse selector.", example: ".btn-primary { … }" },
      { prop: "#id", desc: "Element with the given ID. Specificity 0,1,0,0 — very high. Avoid for styling; prefer classes.", example: "#header { … }" },
      { prop: "el, el", desc: "Group — each selector is evaluated independently and gets the same declarations.", example: "h1, h2, h3 { font-family: serif; }" },
      { prop: "a b", desc: "Descendant combinator — any depth. Broad and cheap-looking but easy to over-scope.", example: "article p { line-height: 1.7; }" },
      { prop: "a > b", desc: "Direct child only.", example: ".menu > li { display: inline-block; }" },
      { prop: "a + b", desc: "Adjacent sibling — B immediately after A.", example: "h2 + p { margin-top: 0; }" },
      { prop: "a ~ b", desc: "General sibling — any B after A at the same level.", example: "input:checked ~ label { color: green; }" },
      {
        prop: "[attr=value]",
      ...
      { prop: ":hover / :focus / :active", desc: "Interaction states. Pair `:focus-visible` with focus styles to avoid mouse-click focus rings.", example: "a:hover, a:focus-visible { text-decoration: underline; }" },
      {
        prop: ":nth-child(n)",
        ...
        gotcha: "Counts against ALL siblings regardless of type. Use `:nth-of-type()` to count within the same tag.",
      },
      { prop: ":not(sel)", desc: "Negation. Modern browsers accept complex selectors and lists.", example: "li:not(.disabled, :first-child) { … }" },
      {
        prop: ":is() / :where()",
      ...
      { prop: "::placeholder / ::selection", desc: "Style input placeholder text and user-highlighted text respectively.", example: "input::placeholder { color: #9ca3af; }" },
    ],
    realWorld: {
      title: "Form Field with Validation State",
      description: "Uses attribute selectors, :focus-visible, :has(), and ::before to style a labeled input based purely on its state.",
...
.field:has(input:invalid:not(:placeholder-shown)) input {
  ...
  },
  {
    ...
    rows: [
      ...
      { prop: ":focus-visible", desc: "Focus only when the UA thinks a ring is warranted (keyboard nav)." },
      { prop: ":active", desc: "Currently being activated (mousedown)." },
      { prop: ":nth-child(n)", desc: "Pick items by index / formula.", example: ":nth-child(2n+1)" },
      ...
      { prop: "::before / ::after", desc: "Inject virtual children.", example: "::before { content: '★'; }" },
  ...
  },
  {
    ...
    rows: [
      ...
      { prop: "@property", desc: "Register a typed custom property so it can animate.", example: "@property --hue { syntax:'<angle>'; inherits:true; initial-value:0deg; }" },
      { prop: "calc() with vars", desc: "Compose values.", example: "padding: calc(var(--gap) * 2);" },
  ...
  },
  {
    ...
    rows: [
      ...
      { prop: "Common breakpoints", desc: "sm 640 · md 768 · lg 1024 · xl 1280 · 2xl 1536 (Tailwind scale)." },
      { prop: "(orientation: portrait)", desc: "Portrait vs landscape." },
      { prop: "(prefers-color-scheme: dark)", desc: "Respect the OS theme.", example: "@media (prefers-color-scheme: dark) { … }" },
  ...
  },
  {
    ...
    rows: [
      ...
      { prop: "counter(name)", desc: "Read a CSS counter value.", example: "content: counter(step);" },
  ...
  },
  {
    ...
    rows: [
      ...
      { prop: "filter: drop-shadow()", desc: "Shadow that follows alpha, not the box.", example: "drop-shadow(0 4px 8px #0007)" },
      { prop: "backdrop-filter", desc: "Apply a filter to what is BEHIND the element (glass effect)." },
      { prop: "mix-blend-mode", desc: "Blend with the element beneath.", example: "mix-blend-mode: multiply;" },
      { prop: "background-blend-mode", desc: "Blend a background layer with the ones below it." },
      { prop: "isolation: isolate", desc: "Create a stacking context so blends do not leak upward." },
    ],
  },
  {
    ...
    rows: [
      ...
      { prop: "animation (shorthand)", desc: "name duration easing delay iter direction fill state.", example: "animation: spin 1s linear infinite;" },
      { prop: "Common easings", desc: "linear · ease · ease-in · ease-out · ease-in-out · cubic-bezier(.2,.8,.2,1) · steps(n)." },
    ],
  },
  {
    ...
    rows: [
      { prop: "grid-template-areas", desc: "Draw the layout as strings.", example: "'header header' 'nav main'" },
      { prop: "grid-area", desc: "Place a child into a named area." },
      { prop: "Named lines", desc: "grid-template-columns: [start] 1fr [mid] 2fr [end];" },
      { prop: "auto-fill", desc: "Create as many tracks as fit, even empty ones.", example: "repeat(auto-fill, minmax(200px, 1fr))" },
      { prop: "auto-fit", desc: "Like auto-fill but collapses empty tracks — items stretch to fill." },
      { prop: "Explicit vs implicit", desc: "Explicit = defined by grid-template-*; implicit = created for overflow items." },
      ...
      { prop: "subgrid", desc: "Inherit parent's tracks.", example: "grid-template-columns: subgrid;" },
    ],
  },
  {
    ...
    rows: [
      ...
      { prop: "scroll-snap-type", desc: "x/y + mandatory/proximity.", example: "scroll-snap-type: x mandatory;" },
      { prop: "scroll-snap-align", desc: "start · center · end — on the children." },
      { prop: "scroll-padding / scroll-margin", desc: "Offset the snap area (e.g. under a sticky header)." },
      { prop: "scrollbar-gutter", desc: "Reserve space so layout does not shift when a scrollbar appears." },
      { prop: "clip-path", desc: "Shape the visible region.", example: "clip-path: polygon(0 0, 100% 0, 100% 80%, 0 100%);" },
  ...
  },
  {
    ...
    rows: [
      { prop: "@media print", desc: "Print-only rules — hide nav, unset backgrounds, force color." },
      { prop: "page-break-inside: avoid", desc: "Keep a block together across pages." },
      { prop: "@page", desc: "Set page size and margins for print." },
      { prop: "prefers-reduced-motion", desc: "Disable non-essential animations for sensitive users." },
      { prop: "prefers-contrast", desc: "more · less · custom — raise or lower contrast." },
      { prop: "forced-colors: active", desc: "Windows High Contrast mode is on — respect system colors." },
      { prop: "forced-color-adjust", desc: "auto · none — opt out of forced-colors for specific elements." },
      { prop: ":focus-visible", desc: "Only show focus rings when useful (keyboard nav)." },
      { prop: ".sr-only pattern", desc: "Visually hidden but readable to screen readers.", example: "position:absolute; width:1px; height:1px; overflow:hidden;" },
      { prop: "color-scheme", desc: "Tell the UA which schemes your page supports.", example: "color-scheme: light dark;" },
  ...
  },
  {
    ...
    rows: [
      { prop: "container-type", desc: "Turn an element into a query container.", example: "container-type: inline-size;" },
      { prop: "container-name", desc: "Name the container for targeted queries." },
      { prop: "@container", desc: "Style based on the container's size, not the viewport.", example: "@container (min-width: 480px) { … }" },
      { prop: ":has(sel)", desc: "Parent/relational selector.", example: "article:has(img) { … }" },
      { prop: "@layer", desc: "Cascade layers — control which rules win regardless of source order.", example: "@layer base, components, utilities;" },
      { prop: "Native nesting", desc: "Nest selectors like Sass, directly in CSS.", example: ".card { & .title { font-weight: 700; } }" },
      ...
      { prop: "view-transitions", desc: "::view-transition-* pseudos + document.startViewTransition()." },
      { prop: "@property", desc: "Register typed custom properties so they can animate/interpolate." },
    ],
  },
];
...
function CheatSheetPage() {
  ...
  React.useEffect(() => {
    try { localStorage.setItem(LS_KEY, String(current)); } catch {}
  }, [current]);
  ...
  const renderPdf = async () => {
      ...
      doc.text(`Generated ${new Date().toLocaleDateString()}`, pageW / 2, pageH / 2 + 30, { align: "center" });
      ...
      <div className="flex flex-col gap-3 rounded-xl border border-border bg-card/60 p-3 sm:flex-row sm:items-center sm:justify-between sm:gap-4 sm:p-4">
        <div className="flex min-w-0 flex-1 flex-col gap-1.5">
          <div className="flex items-center justify-between gap-2 text-xs text-muted-foreground">
            <span>
              Chapter <span className="font-semibold text-foreground">{current + 1}</span> of {SECTIONS.length}
            </span>
        ...
        <DialogContent className="flex h-[92vh] w-[96vw] max-w-6xl flex-col gap-3 p-4 sm:p-5">
          <DialogHeader className="space-y-1">
            ...
            <DialogDescription className="text-xs">
              Exact rendering of the exported PDF — {previewPages} page{previewPages === 1 ? "" : "s"}.
              Page breaks and "(cont.)" continuation markers appear where they will in the download.
            </DialogDescription>
          </DialogHeader>
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,260px)_minmax(0,1fr)]">
        ...
        >
          ...
          >
            ...
            <ol
              className="relative grid gap-1 lg:max-h-[calc(100vh-14rem)] lg:overflow-y-auto lg:pr-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"
            >
              {SECTIONS.map((s, i) => {
                ...
                return (
                  <li key={s.title}>
                    ...
                    >
                      ...
                      {active ? (
                        <ChevronRight className="ml-auto h-3.5 w-3.5 shrink-0 text-primary" aria-hidden="true" />
                      ) : null}
                    </button>
                  </li>
                );
        ...
        <div className="flex min-w-0 flex-col gap-5">
            ...
            {query && crossChapterResults.length > 0 ? (
              ...
              >
                <div className="px-2 py-1.5 text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">
                  {totalMatches} match{totalMatches === 1 ? "" : "es"} across {crossChapterResults.length} chapter{crossChapterResults.length === 1 ? "" : "s"}
                </div>
                <ul className="grid gap-2">
                  {crossChapterResults.map((g) => (
                    <li key={g.chapter} className="rounded-lg border border-border/50 bg-background/50">
                      <button
                        type="button"
                      ...
                      >
                        <span className="flex items-center gap-2 min-w-0">
                          ...
                          {g.chapter === current ? (
                            <span className="ml-1 rounded-full bg-primary/15 px-1.5 py-0.5 text-[9px] font-semibold uppercase tracking-wider text-primary">current</span>
                          ) : null}
                        </span>
                        <span className="shrink-0 text-xs text-muted-foreground">{g.matches.length} match{g.matches.length === 1 ? "" : "es"}</span>
                      </button>
                      <ul className="border-t border-border/50">
                        {g.matches.slice(0, 5).map((m) => (
                          <li key={m.prop}>
                            ...
                            >
                              <CornerDownRight className="mt-0.5 h-3 w-3 shrink-0 text-muted-foreground" aria-hidden="true" />
          ...
          >
            ...
            <div className="p-4 sm:p-5">
              {filteredRows.length === 0 ? (
                ...
                >
                  <div className="text-base font-semibold text-foreground">No matches in this chapter</div>
                    ...
                    return (
                      ...
                      >
                        ...
                        {values && values.length > 0 ? (
                          <div className="mt-1.5">
                            <div className="mb-1 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Values</div>
                            <ul className="grid gap-1">
                              {values.map((v) => (
                                <li key={v.name} className="text-xs leading-relaxed">
                                  ...
                                  <span className="text-muted-foreground"> — <Highlight text={v.meaning} query={query} /></span>
                                </li>
                              ))}
                            </ul>
                          </div>
                        ...
                        {examples && examples.length > 0 ? (
                          <div className="mt-1.5 grid gap-1">
                            {examples.map((ex, i) => (
                              <code key={i} className="block overflow-x-auto rounded-md border border-border/50 bg-muted/50 px-2.5 py-1.5 font-mono text-[11px] leading-relaxed text-foreground">
                        ...
                        {gotcha ? (
                          <div className="mt-1.5 rounded-md border border-amber-500/30 bg-amber-500/5 px-2.5 py-1.5 text-xs leading-relaxed text-amber-800 dark:text-amber-200">
                            <span className="font-semibold">⚠ Gotcha:</span> <Highlight text={gotcha} query={query} />
                          </div>
                        ) : null}
                        {support ? (
                          <div className="mt-1 text-[11px] text-muted-foreground">
                            <span className="font-semibold uppercase tracking-wider">Support:</span> <Highlight text={support} query={query} />
                          </div>
                        ) : null}
                      </div>
                    );
              ...
              {section.realWorld ? (
                <section className="mt-6 overflow-hidden rounded-xl border border-primary/30 bg-gradient-to-br from-primary/5 to-transparent">
                  <header className="border-b border-border/60 px-4 py-3">
                    <div className="text-[10px] font-semibold uppercase tracking-wider text-primary">Real-world example</div>
                    <h3 className="mt-0.5 text-base font-semibold text-foreground sm:text-lg">{section.realWorld.title}</h3>
                    {section.realWorld.description ? (
                      <p className="mt-1 text-xs leading-relaxed text-muted-foreground sm:text-sm">{section.realWorld.description}</p>
                    ) : null}
                  </header>
                  <div className="grid gap-0 md:grid-cols-2">
                    ...
                    <div className="grid gap-2 border-t border-border/60 p-3 md:border-l md:border-t-0">
                      <div>
                        <div className="mb-1 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">HTML</div>
                        <pre className="max-h-48 overflow-auto rounded-md border border-border/50 bg-muted/60 p-2 font-mono text-[11px] leading-relaxed text-foreground"><code>{section.realWorld.html}</code></pre>
                      </div>
                      <div>
                        <div className="mb-1 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">CSS</div>
                        <pre className="max-h-64 overflow-auto rounded-md border border-border/50 bg-muted/60 p-2 font-mono text-[11px] leading-relaxed text-foreground"><code>{section.realWorld.css}</code></pre>
                      </div>
                    </div>
                  </div>
                </section>
          ...
          >
            ...
            >
              <span className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground/70">Ch.</span>
