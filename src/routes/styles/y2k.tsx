  ...
  head: () => ({
    meta: [
      { title: "Y2K / Retro Style Generator — SagaCSS" },
      {
        name: "description",
        content:
          "Design chrome text, bubble buttons and glossy gradients with the SagaCSS Y2K retro style generator.",
      },
...
function Y2KPage() {
  ...
  const applyPreset = (p: (typeof presets)[number]) => {
    setC1(p.c1);
    setC2(p.c2);
    setChrome1(p.chrome1);
    setChrome2(p.chrome2);
    setRadius(p.radius);
    setGlow(p.glow);
  };
...
.y2k-text {
  ...
  return (
    <div className="flex flex-col gap-6">
      <ToolHeader
        title="Y2K / Retro Style"
        description="Chrome text, bubble borders and glossy gradients — instant early-2000s energy."
      />

      <div
        className="flex min-h-[18rem] items-center justify-center rounded-2xl border border-border bg-slate-100 p-8"
        style={{
          background:
            "repeating-linear-gradient(45deg, #f8fafc, #f8fafc 8px, #eef2ff 8px, #eef2ff 16px)",
        }}
      >
        <div
          style={{
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <div className="flex flex-col gap-4 rounded-xl border border-border bg-card p-4 md:p-5">
          ...
          <div>
            <Label htmlFor="y2k-text" className="text-xs">
              Text
            </Label>
            <Input
              id="y2k-text"
