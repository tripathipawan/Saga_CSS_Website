  ...
  head: () => ({
    meta: [
      { title: "Neumorphism Generator — SagaCSS" },
      {
        name: "description",
        content:
          "Design raised and pressed neumorphic elements with dual-shadow controls, light/dark presets, and copy-ready CSS.",
      },
  ...
  return (
    <div className="flex flex-col gap-6">
      <ToolHeader
        ...
        description="Design soft, extruded UI with dual light/dark shadows. Toggle raised vs pressed for interactive states."
      />

      <div
        className="flex min-h-[15rem] max-h-[17rem] items-center justify-center rounded-2xl border border-border p-10"
        style={{ background: bg }}
      >
        <div
          className="h-40 w-40 md:h-48 md:w-48"
          style={{ background: bg, borderRadius: radius, boxShadow: shadow }}
        ...
        <div className="rounded-xl border border-border bg-card p-4 md:p-5">
          <div className="mb-4 grid grid-cols-[auto_1fr] items-center gap-3">
            ...
            <div className="text-xs text-muted-foreground">
              Base color — the surface the element sits on
            </div>
          </div>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {(
              [
                ...
                { label: "Radius", val: radius, set: setRadius, min: 0, max: 80 },
              ] as const
            ).map((c) => (
              <div key={c.label}>
            ...
            <div className="flex items-center gap-2 sm:col-span-2">
              <Switch
                ...
                aria-label="Pressed state"
              />
              <Label htmlFor="pressed" className="text-sm">
                Pressed (inset)
              </Label>
            </div>
          </div>
