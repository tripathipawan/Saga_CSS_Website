  ...
  head: () => ({
    meta: [
      { title: "Neubrutalism Generator — SagaCSS" },
      {
        name: "description",
        content:
          "Design bold neubrutalist buttons and cards with thick borders, hard offset shadows and shadow-shift hover interaction.",
      },
...
&:hover {
  ...
  return (
    <div className="flex flex-col gap-6">
      <ToolHeader
        title="Neubrutalism Generator"
        description="Design bold, high-contrast components with thick borders and hard shadows that shift on hover."
      />
      ...
      <div className="flex min-h-[15rem] max-h-[17rem] items-center justify-center rounded-2xl border border-border bg-[repeating-linear-gradient(45deg,#f8fafc,#f8fafc_10px,#e2e8f0_10px,#e2e8f0_20px)] p-8 dark:bg-[repeating-linear-gradient(45deg,#0f172a,#0f172a_10px,#1e293b_10px,#1e293b_20px)]">
        <button
          ...
          style={{
            ...
            border: `${borderWidth}px solid ${border}`,
            borderRadius: radius,
            boxShadow: `${offX}px ${offY}px 0 0 ${border}`,
            fontWeight: weight,
          }}
          onMouseEnter={(e) => {
