  ...
  head: () => ({
    meta: [
      { title: "Font Pairing & Google Fonts Explorer | SagaCSS" },
      {
        name: "description",
        content:
          "Browse curated Google Fonts by category, pair heading and body fonts, and copy the <link>, CSS font-family and Tailwind config snippets in one click.",
      },
...
const PAIRINGS: { heading: string; body: string; note: string }[] = [
  {
    ...
    body: "Inter",
    note: "Editorial serif + neutral sans — classic magazine look.",
  },
  { heading: "Poppins", body: "Roboto", note: "Modern geometric sans pairing — great for SaaS." },
  {
    heading: "Bebas Neue",
    body: "Work Sans",
    note: "Bold display + friendly sans — punchy landing pages.",
  },
  {
    heading: "Abril Fatface",
    body: "Lora",
    note: "High-contrast serif + humanist serif — elegant and readable.",
  },
  {
    heading: "Montserrat",
    body: "Merriweather",
    note: "Geometric sans headings + traditional serif body.",
  },
  {
    heading: "DM Sans",
    body: "DM Sans",
    note: "Same-family pairing with weight contrast — safe and modern.",
  },
  { heading: "Anton", body: "Roboto", note: "Condensed display + neutral body — bold and clean." },
  {
    heading: "Cormorant Garamond",
    body: "Montserrat",
    note: "Classical serif headings + modern sans body.",
  },
];
...
function FontsPage() {
  ...
  return (
    <div className="flex flex-col gap-6">
      <ToolHeader
        title="Font Pairing & Google Fonts"
        description="Browse curated Google Fonts, load proven heading + body pairings, then copy the link tag and CSS in one shot."
      />
      ...
      <div className="rounded-2xl border border-border bg-gradient-to-br from-primary/5 to-accent/5 p-6">
        ...
        >
          {preview}. This paragraph uses the body font while the heading above uses the display face
          — the pairing shows up as visual hierarchy and typographic contrast.
        </p>
        <Input
          value={preview}
            ...
            <div className="mb-2 flex flex-wrap gap-1">
              {categories.map((c) => (
                <button
                  ...
                  className={`rounded-full border px-2.5 py-1 text-xs ${category === c ? "border-primary bg-primary text-primary-foreground" : "border-border hover:bg-accent"}`}
                >
                  {c}
                </button>
              ))}
              ...
              {filtered.map((f) => (
                ...
                >
                  <span
                    ...
                    style={{
                      fontFamily: `"${f.family}", ${f.category === "Serif" ? "serif" : f.category === "Monospace" ? "monospace" : "sans-serif"}`,
                    }}
                  >
                    {f.name}
                  </span>
                  <div className="flex gap-1">
                    <button
                      ...
                      aria-label={`Use ${f.name} as heading`}
                    >
                      H
                    </button>
                    <button
