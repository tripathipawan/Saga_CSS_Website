  ...
  head: () => ({
    meta: [
      { title: "CSS Specificity Visualizer — Debug Selector Conflicts — SagaCSS" },
      {
        name: "description",
        content:
          "Paste any CSS selector to see its specificity score broken down visually. Compare two selectors side by side to know which one wins a conflict.",
      },
...
function SpecificityPage() {
  ...
  const saveA = () => {
    ...
    addSnippet({
      ...
      code: `/* Selector: ${a}\nSpecificity: ${fmt(spA)}\n*/`,
      format: "css",
      source: "Specificity Visualizer",
      sourcePath: "/tools/specificity",
    });
    toast.success("Saved to My Kit");
  };

  const verdict =
    spA && spB ? (compare(spA, spB) > 0 ? "A" : compare(spA, spB) < 0 ? "B" : "TIE") : null;
    ...
    <div className="mx-auto max-w-4xl">
        ...
        <div className="mt-4 rounded-xl border border-border bg-card p-4 text-center">
          {verdict === "TIE" && (
            <div className="text-sm">
              ...
              <strong className="font-semibold">({fmt(spA!)})</strong>. The rule declared{" "}
              <em>later</em> in the cascade wins.
            </div>
          )}
          {verdict === "A" && (
            <div className="text-sm inline-flex items-center gap-2 flex-wrap justify-center">
              <ArrowLeft className="h-4 w-4 text-primary" />{" "}
              <strong className="font-semibold text-primary">Selector A wins</strong> —{" "}
              <span className="text-muted-foreground">{fmt(spA!)}</span> beats{" "}
              <span className="text-muted-foreground">{fmt(spB!)}</span>
            </div>
          )}
          {verdict === "B" && (
            <div className="text-sm inline-flex items-center gap-2 flex-wrap justify-center">
              <strong className="font-semibold text-primary">Selector B wins</strong>{" "}
              <ArrowRight className="h-4 w-4 text-primary" /> —{" "}
              <span className="text-muted-foreground">{fmt(spB!)}</span> beats{" "}
              <span className="text-muted-foreground">{fmt(spA!)}</span>
            </div>
          )}
        </div>
      )}
      ...
      <section className="mt-6 rounded-xl border border-border bg-card p-4 text-sm">
        ...
        <p className="text-muted-foreground">
          CSS specificity is a 3-part score{" "}
          <code className="rounded bg-muted px-1 py-0.5 text-xs">A,B,C</code>:
        </p>
        <ul className="mt-2 space-y-1 text-muted-foreground">
          <li>
            <span className="mr-2 inline-block h-2 w-2 rounded-full bg-rose-500" />{" "}
            <strong className="text-foreground">A</strong> — IDs (
            <code className="text-xs">#foo</code>)
          </li>
          <li>
            <span className="mr-2 inline-block h-2 w-2 rounded-full bg-amber-500" />{" "}
            <strong className="text-foreground">B</strong> — classes, attributes, pseudo-classes (
            <code className="text-xs">.foo</code>, <code className="text-xs">[type]</code>,{" "}
            <code className="text-xs">:hover</code>)
          </li>
          <li>
            <span className="mr-2 inline-block h-2 w-2 rounded-full bg-emerald-500" />{" "}
            <strong className="text-foreground">C</strong> — elements and pseudo-elements (
            <code className="text-xs">div</code>, <code className="text-xs">::before</code>)
          </li>
        </ul>
        <p className="mt-2 text-muted-foreground">
          <code className="text-xs">:where()</code> contributes zero.{" "}
          <code className="text-xs">:is()</code>, <code className="text-xs">:not()</code>, and{" "}
          <code className="text-xs">:has()</code> take the highest specificity of their arguments.
          Inline styles add another level above everything (<code className="text-xs">1,0,0,0</code>
          ).
        </p>
      </section>
          ...
          <div className="flex flex-wrap gap-2">
            {history.map((h) => (
              <button
                key={h}
                onClick={() => setA(h)}
                className="rounded-full border border-border bg-card px-3 py-1 text-xs font-mono hover:border-primary/40"
              >
                {h}
              </button>
            ))}
  ...
  return (
    <div className="rounded-xl border border-border bg-card p-4">
      ...
      <Input
        ...
        placeholder="e.g. #header .nav > ul li:hover"
      />
      {sp && (
        <>
          <div className="mt-3 flex items-center justify-between gap-2">
            ...
            <Button
              ...
              variant="ghost"
              onClick={onSave}
              aria-label="Save selector to My Kit"
          ...
          <div className="mt-3 flex flex-wrap gap-1.5">
            {sp.parts.map((t, i) => (
              <span
                key={i}
                className={`rounded border px-1.5 py-0.5 text-xs font-mono ${KIND_COLORS[t.kind]}`}
                title={t.kind}
              >
                {t.text}
              </span>
          ...
          <div className="mt-3 grid grid-cols-3 gap-2 text-center text-xs">
            <div className="rounded bg-rose-500/10 py-1">
              <div className="font-semibold">{sp.a}</div>
              <div className="text-muted-foreground">IDs</div>
            </div>
            <div className="rounded bg-amber-500/10 py-1">
              <div className="font-semibold">{sp.b}</div>
              <div className="text-muted-foreground">Class/Attr/Pseudo</div>
            </div>
            <div className="rounded bg-emerald-500/10 py-1">
              <div className="font-semibold">{sp.c}</div>
