  ...
  head: () => ({
    meta: [
      { title: "Color Converter — HEX RGB HSL HWB CMYK LCH | SagaCSS" },
      {
        name: "description",
        content:
          "Convert any color between HEX, RGB, RGBA, HSL, HSLA, HWB, CMYK, LCH, XYZ and named CSS colors with live sync between picker, sliders and hex input.",
      },
...
function rgbToXyz(r: number, g: number, b: number) {
  ...
  return {
    ...
    y: +(R * 0.2126729 + G * 0.7151522 + B * 0.072175).toFixed(4) * 100,
    z: +(R * 0.0193339 + G * 0.119192 + B * 0.9503041).toFixed(4) * 100,
  };
}
...
function Row({ label, value }: { label: string; value: string }) {
  ...
  return (
    <div className="flex items-center gap-2 rounded-md border border-border bg-background/50 p-2">
      <div className="w-20 text-xs font-medium uppercase tracking-wide text-muted-foreground">
        {label}
      </div>
  ...
  return (
    <div className="flex flex-col gap-6">
      <ToolHeader
        ...
        description="Convert any color between HEX, RGB, HSL, HWB, CMYK, LCH, XYZ and the nearest CSS named color — all synced."
      />

      <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.2fr)]">
        <div className="flex flex-col gap-4">
          <div
            className="relative flex h-56 items-center justify-center overflow-hidden rounded-2xl border border-border shadow-inner"
            style={{ background: hex }}
            aria-label={`Color preview ${hex}`}
            role="img"
          >
            <span className="rounded-md bg-black/40 px-3 py-1 font-mono text-sm text-white backdrop-blur-sm">
              {hex.toUpperCase()}
            </span>
          </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <Input
                ...
                onChange={(e) => {
                  ...
                  if (/^#?[0-9a-fA-F]{0,6}$/.test(v)) setHex(v.startsWith("#") ? v : `#${v}`);
                }}
                className="font-mono"
