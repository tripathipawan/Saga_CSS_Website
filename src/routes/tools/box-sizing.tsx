...
export const Route = createFileRoute("/tools/box-sizing")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Visualize the CSS box model with live width, padding, margin and border controls and copy the CSS." },
      { property: "og:title", content: "Box Sizing Tool — SagaCSS" },
      { property: "og:description", content: "Interactive CSS box model visualizer with copy-ready code." },
...
box-sizing: border-box;`,
  ...
  const sizeStep = (n: number) => (n === 0 ? "0" : n <= 4 ? "1" : n <= 8 ? "2" : n <= 16 ? "3" : n <= 24 ? "4" : "5");
  const borderCls = border === 0 ? "border-0" : border <= 2 ? "border" : border <= 3 ? "border border-2" : border <= 4 ? "border border-3" : border <= 5 ? "border border-4" : "border border-5";
  ...
  return (
    <div className="flex flex-col gap-6">
      <ToolHeader title="Box Model & Sizing" description="Adjust width, padding, margin and border, then copy the CSS with the DevTools-style box diagram." />
      ...
      <div className="flex min-h-[16rem] max-h-[18rem] max-h-[32rem] items-center justify-center overflow-hidden rounded-2xl border border-border bg-gradient-to-br from-primary/5 to-accent/5 p-6">
        <div className="relative max-w-full max-h-full overflow-hidden rounded-md" style={{ padding: margin, background: "rgba(251, 191, 36, 0.15)" }}>
          <div className="absolute left-1 top-1 text-[10px] font-mono text-yellow-700 dark:text-yellow-300">margin {margin}px</div>
          <div style={{ border: `${border}px solid #8b5cf6` }}>
            <div className="relative" style={{ padding: pad, background: "rgba(34, 211, 238, 0.15)" }}>
              <div className="absolute left-1 top-1 text-[10px] font-mono text-cyan-700 dark:text-cyan-300">padding {pad}px</div>
              <div className="flex items-center justify-center overflow-hidden rounded bg-primary/80 text-primary-foreground font-mono text-xs" style={{ width: `${w}${unit}`, height: `${h}${unit}`, maxWidth: "100%", maxHeight: "24rem" }}>
                content {w}{unit} × {h}{unit}
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-xl border border-border bg-card p-4 md:p-5">
          <div className="mb-4 flex items-center gap-2">
            ...
            <div className="flex gap-1 rounded-md border border-border p-1">
              {UNITS.map((u) => (
                <button key={u} type="button" onClick={() => changeUnit(u)} aria-pressed={unit === u} className={`rounded px-2 py-1 text-xs ${unit === u ? "bg-primary text-primary-foreground" : "text-muted-foreground"}`}>{u}</button>
              ))}
            </div>
          </div>

          <div className="grid gap-4 sm:grid-cols-2">
            {([{ label: "Width", val: w, set: setW, min: UNIT_RANGES[unit].min, max: UNIT_RANGES[unit].max, step: unit === "px" ? 1 : 0.5 },
               { label: "Height", val: h, set: setH, min: UNIT_RANGES[unit].min, max: UNIT_RANGES[unit].max, step: unit === "px" ? 1 : 0.5 },
               { label: "Padding", val: pad, set: setPad, min: 0, max: 80, step: 1 },
               { label: "Margin", val: margin, set: setMargin, min: 0, max: 80, step: 1 },
               { label: "Border", val: border, set: setBorder, min: 0, max: 20, step: 1 }] as const).map((c) => (
              <div key={c.label}>
                <div className="mb-1 flex items-center justify-between">
                  <Label className="text-xs">{c.label}</Label>
                  <span className="font-mono text-xs text-muted-foreground">{c.val}{c.label === "Width" || c.label === "Height" ? unit : "px"}</span>
                </div>
