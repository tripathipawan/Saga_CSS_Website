...
export const Route = createFileRoute("/tools/button")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Design custom CSS buttons visually — padding, radius, gradients, shadow and hover interaction. Copy production-ready CSS." },
...
function ButtonPage() {
  ...
  const presets = [
    { name: "Primary Gradient", padX: 20, padY: 12, radius: 10, color1: "#8b5cf6", color2: "#22d3ee", gradient: true, textColor: "#ffffff", shadow: 20, hoverLift: 3 },
    { name: "Ghost Outline", padX: 18, padY: 10, radius: 8, color1: "#ffffff", color2: "#ffffff", gradient: false, textColor: "#111827", shadow: 0, hoverLift: 0 },
    { name: "Neon Pop", padX: 24, padY: 14, radius: 999, color1: "#22d3ee", color2: "#a855f7", gradient: true, textColor: "#ffffff", shadow: 50, hoverLift: 4 },
    { name: "Neumorphic", padX: 24, padY: 12, radius: 14, color1: "#e0e5ec", color2: "#e0e5ec", gradient: false, textColor: "#334155", shadow: 30, hoverLift: 0 },
    { name: "Sunset CTA", padX: 28, padY: 14, radius: 12, color1: "#f97316", color2: "#ef4444", gradient: true, textColor: "#ffffff", shadow: 30, hoverLift: 4 },
    { name: "Ink Pill", padX: 22, padY: 10, radius: 999, color1: "#0f172a", color2: "#0f172a", gradient: false, textColor: "#ffffff", shadow: 14, hoverLift: 2 },
  ];
  const apply = (p: typeof presets[number]) => {
    setPadX(p.padX); setPadY(p.padY); setRadius(p.radius); setColor1(p.color1); setColor2(p.color2);
    setGradient(p.gradient); setTextColor(p.textColor); setShadow(p.shadow); setHoverLift(p.hoverLift);
  };
...
.btn:hover {
  ...
  const bootstrap = useMemo(() => {
    ...
    const shadowCls = shadow >= 24 ? "shadow-lg" : shadow >= 8 ? "shadow" : shadow > 0 ? "shadow-sm" : "shadow-none";
    const paddingCls =
      padY >= 20 ? "py-4" : padY >= 14 ? "py-3" : padY >= 10 ? "py-2" : "py-1";
    const paddingXCls =
      padX >= 32 ? "px-5" : padX >= 24 ? "px-4" : padX >= 16 ? "px-3" : "px-2";
  ...
  return (
    <div className="flex flex-col gap-6">
      <ToolHeader title="Button Style Generator" description="Compose a custom CSS button with padding, radius, gradient background, shadow and hover lift — hover the preview to try it." />
      ...
      <div className="flex min-h-[16rem] max-h-[18rem] items-center justify-center overflow-hidden rounded-2xl border border-border bg-gradient-to-br from-primary/5 to-accent/5 p-6">
        <button
          type="button"
          style={{
            background: bg, color: textColor, padding: `${padY}px ${padX}px`, border: 0, borderRadius: radius, fontWeight: 600, cursor: "pointer",
            boxShadow: `0 ${Math.round(shadow / 2)}px ${shadow}px -${Math.round(shadow / 3)}px ${color1}55`, transition: "transform 180ms ease, box-shadow 180ms ease",
          }}
          onMouseEnter={(e) => {
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <div className="rounded-xl border border-border bg-card p-4">
          ...
          <div className="mb-3">
            ...
            <Input value={label} onChange={(e) => setLabel(e.target.value)} className="h-9" aria-label="Button label" />
          </div>
          <div className="grid gap-3 sm:grid-cols-2">
            {([{ label: "Padding X", val: padX, set: setPadX, min: 8, max: 60 },
               ...
               { label: "Hover lift", val: hoverLift, set: setHoverLift, min: 0, max: 10 }] as const).map((c) => (
              <div key={c.label}>
            ...
            <div className="sm:col-span-2 flex flex-wrap items-center gap-3">
              <div className="flex items-center gap-2">
                <Switch id="grad" checked={gradient} onCheckedChange={setGradient} aria-label="Use gradient background" />
                <Label htmlFor="grad" className="text-sm">Gradient</Label>
              </div>
              <div className="flex items-center gap-1"><Label className="text-xs">Color 1</Label><input type="color" value={color1} onChange={(e) => setColor1(e.target.value)} className="h-8 w-9 cursor-pointer rounded border border-border bg-transparent" aria-label="Color 1" /></div>
              <div className="flex items-center gap-1"><Label className="text-xs">Color 2</Label><input type="color" value={color2} onChange={(e) => setColor2(e.target.value)} className="h-8 w-9 cursor-pointer rounded border border-border bg-transparent" aria-label="Color 2" /></div>
              <div className="flex items-center gap-1"><Label className="text-xs">Text</Label><input type="color" value={textColor} onChange={(e) => setTextColor(e.target.value)} className="h-8 w-9 cursor-pointer rounded border border-border bg-transparent" aria-label="Text color" /></div>
            </div>
          </div>
        </div>
