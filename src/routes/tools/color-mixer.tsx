  ...
  head: () => ({
    meta: [
      { title: "Color Mixer — Blend Two Colors | SagaCSS" },
      {
        name: "description",
        content:
          "Mix any two colors with adjustable ratio, switch between RGB and HSL mixing modes, click the spectrum bar to sample any midpoint.",
      },
...
function MixerPage() {
  ...
  const spectrum = useMemo(
    () => Array.from({ length: 24 }, (_, i) => mix(a, b, i / 23, mode)),
    [a, b, mode],
  );
  ...
  return (
    <div className="flex flex-col gap-6">
      <ToolHeader
        title="Color Mixer"
        description="Blend two colors with an adjustable ratio and switch between RGB and HSL interpolation modes."
      />

      <div
        className="relative flex flex-col items-center justify-center gap-4 overflow-hidden rounded-2xl border border-border p-6"
      ...
      >
        <div className="grid w-full max-w-xl grid-cols-3 items-center gap-3">
          <div
            ...
            style={{ background: a, aspectRatio: "1" }}
            aria-label={`Color A ${a}`}
            role="img"
          />
          <div
            className="rounded-xl border-2 border-primary shadow-lg"
            style={{ background: result, aspectRatio: "1" }}
            aria-label={`Mixed color ${result}`}
            role="img"
          />
          <div
            className="rounded-xl border border-border shadow-inner"
            style={{ background: b, aspectRatio: "1" }}
            aria-label={`Color B ${b}`}
            role="img"
          />
        </div>
        <div className="font-mono text-xl font-semibold" style={{ color: result }}>
          {result.toUpperCase()}
        </div>
      </div>
        ...
        <div className="flex flex-col gap-4 rounded-xl border border-border bg-card p-4">
            ...
            <div className="flex gap-1 rounded-md border border-border p-1">
              {(["rgb", "hsl"] as Mode[]).map((m) => (
                <button
                  ...
                  onClick={() => setMode(m)}
                  aria-pressed={mode === m}
                  className={`rounded px-3 py-1 text-xs uppercase ${mode === m ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:bg-accent"}`}
                >
                  {m}
                </button>
              ))}
            ...
            <div className="flex h-12 w-full overflow-hidden rounded-md border border-border">
              {spectrum.map((c, i) => (
                <button
                  key={i}
                  type="button"
                  onClick={() => setRatio(Math.round((i / (spectrum.length - 1)) * 100))}
          ...
          <Button
            ...
            onClick={() => {
              ...
              toast.success("Mixed color copied");
            }}
