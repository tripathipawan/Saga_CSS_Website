...
export const Route = createFileRoute("/tools/text-shadow")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Design layered CSS text-shadows with neon, 3D and retro presets. Live preview with custom text.",
      },
      { property: "og:title", content: "Text Shadow Generator — SagaCSS" },
      { property: "og:description", content: "Multi-layer text-shadow generator with live preview." },
...
const nl = (p: Partial<Layer> = {}): Layer => ({ id: uid(), x: 2, y: 2, blur: 4, color: "#000000", alpha: 0.5, ...p });
...
function TextShadowPage() {
  ...
  return (
    <div className="flex flex-col gap-6">
      <ToolHeader title="Text Shadow Generator" description="Design neon glows, 3D extrusions and retro type effects with layered text-shadows." />
          ...
          <div className="rounded-xl border border-border bg-card p-4">
            <div className="grid gap-3 sm:grid-cols-2">
              <div className="sm:col-span-2">
                <Label htmlFor="tx" className="mb-1 block text-xs">Preview text</Label>
                <Input id="tx" value={text} onChange={(e) => setText(e.target.value)} />
              </div>
              <div>
                <Label className="mb-1 block text-xs">Text color</Label>
                <input type="color" value={color} onChange={(e) => setColor(e.target.value)} aria-label="Text color" className="h-9 w-full cursor-pointer rounded border border-border bg-transparent" />
              </div>
              <div>
                <Label className="mb-1 block text-xs">Background</Label>
                <input type="color" value={bg} onChange={(e) => setBg(e.target.value)} aria-label="Background color" className="h-9 w-full cursor-pointer rounded border border-border bg-transparent" />
              </div>
              <div className="sm:col-span-2">
                ...
                <Slider value={[size]} min={24} max={200} step={1} onValueChange={(v) => setSize(v[0] ?? 96)} aria-label="Font size" />
              </div>
            </div>
          </div>
          ...
          {layers.map((l, idx) => (
            <div key={l.id} className="rounded-xl border border-border bg-card p-4">
              <div className="mb-3 flex items-center justify-between">
                ...
                <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => remove(l.id)} aria-label="Remove layer" disabled={layers.length <= 1}>
              ...
              <div className="grid gap-3 sm:grid-cols-3">
                {([
                  ...
                  <div key={key}>
                    ...
                    <Slider value={[l[key]]} min={min} max={max} step={1} onValueChange={(v) => update(l.id, { [key]: v[0] ?? 0 })} aria-label={label} />
                  </div>
                ))}
              </div>
              <div className="mt-3 grid grid-cols-[auto_1fr_auto] items-center gap-2">
                <input type="color" value={l.color} onChange={(e) => update(l.id, { color: e.target.value })} aria-label="Layer color" className="h-8 w-10 cursor-pointer rounded border border-border bg-transparent" />
                <Input value={l.color} onChange={(e) => { const v = e.target.value; if (/^#[0-9a-fA-F]{0,6}$/.test(v)) update(l.id, { color: v }); }} aria-label="Layer hex" className="h-8 font-mono text-sm" />
                <div className="flex items-center gap-2">
                  <Label className="text-xs text-muted-foreground">α</Label>
                  <Input type="number" min={0} max={1} step={0.05} value={l.alpha} onChange={(e) => update(l.id, { alpha: Math.min(1, Math.max(0, Number(e.target.value) || 0)) })} aria-label="Alpha" className="h-8 w-20 text-right" />
                </div>
              </div>
            </div>
          ))}
          <Button variant="outline" size="sm" onClick={() => setLayers((p) => [...p, nl()])} className="gap-1.5 self-start">
              ...
              {PRESETS.map((p) => (
                ...
                >
                  <span
                    ...
                    style={{ color: p.color, textShadow: p.layers.map((l) => toCss({ ...l, id: "" })).join(", ") }}
                  >
                    {p.name}
                  </span>
                </button>
