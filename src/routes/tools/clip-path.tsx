...
export const Route = createFileRoute("/tools/clip-path")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Visual polygon clip-path editor with draggable points and preset shapes like triangle, hexagon, star and arrow." },
...
const PRESETS: { name: string; points: Pt[] }[] = [
  ...
  { name: "Arrow", points: [{ x: 0, y: 20 }, { x: 60, y: 20 }, { x: 60, y: 0 }, { x: 100, y: 50 }, { x: 60, y: 100 }, { x: 60, y: 80 }, { x: 0, y: 80 }] },
  { name: "Chevron", points: [{ x: 75, y: 0 }, { x: 100, y: 50 }, { x: 75, y: 100 }, { x: 0, y: 100 }, { x: 25, y: 50 }, { x: 0, y: 0 }] },
];
...
function ClipPathPage() {
  ...
  const clip = useMemo(() => `polygon(${points.map((p) => `${p.x}% ${p.y}%`).join(", ")})`, [points]);
  ...
  const nudge = (i: number, dx: number, dy: number) => {
    setPoints((prev) => prev.map((p, j) => (j === i
  ...
  const onKeyDown = (i: number) => (e: React.KeyboardEvent<SVGCircleElement>) => {
    ...
    switch (e.key) {
      case "ArrowUp": e.preventDefault(); nudge(i, 0, -step); break;
      case "ArrowDown": e.preventDefault(); nudge(i, 0, step); break;
      case "ArrowLeft": e.preventDefault(); nudge(i, -step, 0); break;
      case "ArrowRight": e.preventDefault(); nudge(i, step, 0); break;
    }
  };

  return (
    <div className="flex flex-col gap-6">
      <ToolHeader title="Clip Path Generator" description="Drag vertices to shape a CSS polygon clip-path. Load preset shapes and copy production-ready code." />
      ...
      <div className="relative flex min-h-[16rem] items-center justify-center overflow-hidden rounded-2xl border border-border bg-gradient-to-br from-primary/5 to-accent/5 p-6">
        <div className="relative h-56 w-56">
          <div className="absolute inset-0 bg-gradient-to-br from-primary to-accent" style={{ clipPath: clip, WebkitClipPath: clip }} aria-label="Clip path preview" role="img" />
          <svg className="absolute inset-0 h-full w-full touch-none" viewBox="0 0 100 100" preserveAspectRatio="none">
            <polygon points={points.map((p) => `${p.x},${p.y}`).join(" ")} fill="none" stroke="currentColor" strokeWidth={0.4} className="text-primary/60" vectorEffect="non-scaling-stroke" />
            {points.map((p, i) => (
              <circle
                key={i}
                cx={p.x}
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <div className="rounded-xl border border-border bg-card p-4">
          ...
          <p className="mt-3 text-xs text-muted-foreground">Tip: drag the dots on the preview to reshape the polygon.</p>
        </div>
